...
fn decode_shannon_messages(mut decoder: ShannonDecoder<TcpStream>, dispatch: Sender<DispatchCmd>) {
    loop {
        match decoder.decode() {
            Ok(msg) => {
                if dispatch.send(DispatchCmd::Msg(msg)).is_err() {
                    break;
                }
            }
            Err(err) => {
...
fn dispatch_messages(
    ...
    for disp in dispatch {
        match disp {
            DispatchCmd::MercuryReq { callback, request } => {
                let msg = mercury.enqueue_request(callback, request);
            ...
            DispatchCmd::AudioKeyReq {
                ...
                let msg = audio_key.enqueue_request(callback, track, file);
                let _ = messages.send(msg);
            }
            DispatchCmd::CountryCodeReq { callback } => {
                let _ = callback.send(country_code.clone());
            }
            DispatchCmd::Msg(msg) if msg.cmd == ShannonMsg::PING => {
                let _ = messages.send(pong_message());
            }
            DispatchCmd::Msg(msg) if msg.cmd == ShannonMsg::COUNTRY_CODE => {
                country_code.replace(parse_country_code(msg).unwrap());
            }
            DispatchCmd::Msg(msg) if msg.cmd == ShannonMsg::AES_KEY => {
                audio_key.handle_aes_key(msg)
            }
            DispatchCmd::Msg(msg) if msg.cmd == ShannonMsg::AES_KEY_ERROR => {
                audio_key.handle_aes_key_error(msg)
            }
            DispatchCmd::Msg(msg) if msg.cmd == ShannonMsg::MERCURY_REQ => {
