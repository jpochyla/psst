...
use super::{album, artist, playable, playlist, show, theme, track};
...
fn uniquely_yours_results_widget() -> impl Widget<WithCtx<MixedView>> {
    Either::new(
        ...
        Empty,
        Flex::column()
            .with_child(home_header_widget("Uniquely yours"))
            .with_child(
...
fn title_label() -> impl Widget<WithCtx<MixedView>> {
    Either::new(
        ...
        Empty,
        home_header_widget(|data: &Arc<str>, _: &_| data.to_string()),
    )
    ...
    Async::new(
        ...
        || {
            Either::new(
                ...
                Empty,
                Flex::column()
                    .with_child(home_header_widget("Your top artists"))
                    .with_child(
                        Scroll::new(List::new(|| artist::artist_widget(true)).horizontal())
                            .horizontal(),
                    ),
...
fn top_tracks_widget() -> impl Widget<WithCtx<Vector<Arc<Track>>>> {
    Either::new(
        ...
        Empty,
        Flex::column()
            .with_child(home_header_widget("Your top tracks"))
...
fn home_header_widget<T: Data>(text: impl Into<LabelText<T>>) -> impl Widget<T> {
    Label::new(text)
        .with_font(theme::UI_FONT_MEDIUM)
        .with_text_size(theme::grid(2.0))
